...
**Last reviewed:** 2026-09-14 (sweep)
...
- ⚠ **Cloud → NAS access path still failing — 42 failed nightly runs spanning 2026-07-23 → 2026-09-13, all `websocket: bad handshake`** (36 through 09-06 per the prior count, plus 09-07 → 09-13 with 09-11 skipped; each new snapshot verified FAILED). Cadence steady ≈12:38–12:42 UTC. No DB data has been read since the nightly job began. The headline-diagnosis wobble continues on the same pattern: probe-bearing nights 09-07/09-08/09-12 (differential probe: 403 without token, 502 with token) say NAS-side connector down, while no-probe nights 09-09/09-10/09-13 assert Access/edge-layer (09-10 even asserts the failure is "BEFORE the NAS/connector"). The probe-bearing nights remain consistent with the 08-31 diagnosis below; adjudicate at the fix. (sweep-reconciled 2026-09-14)
...
## Next action
- Restart/verify the NAS-side cloudflared connector (per the 08-23 → 08-31 snapshots the token is not the problem); the next nightly snapshot verifies the fix. While in the scheduler, note the accumulating gaps: weekly sweep skipped 08-24, meridia-core skipped 08-29 and 09-11, ALL scheduled jobs were dark on 09-11, and market-scout has not committed since 09-08. Worth doing before the next Hypomone or corpus session, both of which need this path.
